...
| [`rules/03_DOC上稿單填寫與排版規範.md`](./rules/03_DOC上稿單填寫與排版規範.md) | **Word 上稿單排版規範手冊**：集中說明動態欄位純黑無底色要求、圖說格式與原始 URL 規範。 |
| [`templates/新聞編譯標準發稿範本.md`](./templates/新聞編譯標準發稿範本.md) | 包含 11-Step 標準空白發稿表格，以及一篇完整的真實報導示範。 |
| [`templates/新住民全球新聞網_上稿單範本.docx`](./templates/新住民全球新聞網_上稿單範本.docx) | 依據公司原始上稿單轉換之標準空白 Word 範本（無特殊底色，圖說格式為 `{主圖說明(Ảnh envato/{圖片名稱})}`）。 |
...
| `YYYYMMDD/` (如 [`20260906/`](./20260906/)) | **每日新聞產出目錄**：<br>• 外層：存放兩張約 516*292 等比例新聞配圖。<br>• `AI文字內容/`：每日兩篇符合 11 步 SOP 之新聞文字稿件。<br>• `DOC範本/`：**僅存放當日填寫完成之正式交付 Word 文件**（無冗餘範本與說明檔）。 |
...
flowchart TD
    ...
    H --> I[STEP 8: 組合最終 Tag]
    I --> J[STEP 9: 規範圖說與版權: 主圖說明(Ảnh envato/名稱)]
    J --> K[STEP 10: 四維度風險評級]
    K --> L[STEP 11: 新住民適配性 1~5星評定]
    L --> M[產出標準發稿文字]
    M --> N[自動填入 Word 上稿單: 標題/主圖/關鍵字/原始URL/內文]
```
...
2. **新增/異動範本檔案**：若在 `templates/` 中調整發稿結構、Docx 範本或淘汰舊範本，需同步更新本檔「SOP 流程」與「模板索引」。
...
* **2026-09-06 (Docx 樣式優化、原始 URL 修正與架構精簡)**：
  * 淘汰舊版 `templates/AI_改寫指令Prompt範本.md`。
  * 建立 [`rules/03_DOC上稿單填寫與排版規範.md`](./rules/03_DOC上稿單填寫與排版規範.md)，集中規範 Word 上稿單各欄位填寫要求。
  * 清理 `20260906/DOC範本/`，移除重複放置的空白範本與說明檔，確保該目錄專注存放完成成果檔。
  * 優化 Word 上稿單：動態調整欄位去除紫色底色與螢光反白，改為純黑標準字型；主圖圖說格式統一為 `{主圖說明(Ảnh envato/{圖片名稱})}`。
  * 修正新聞 01 連結為真實原始 URL（`https://tuoitre.vn/...`），杜絕使用越文文字描述網址。
  * 重新產出正式 Word 文件（`VN20260906_01_越南新身分法.docx`、`VN20260906_02_海外越語推廣日.docx`）。
  * 同步更新 `AGENT.md` 與 `README.md`。
* **2026-09-06 (Docx 範本與自動化上稿支援)**：
  * 分析公司原始上稿單 `VN090602.docx`，釐清動態調整欄位與固定表頭。
  * 產出標準 Word 範本 `templates/新住民全球新聞網_上稿單範本.docx`。
  * 開發 `scripts/generate_docx_from_news.py` 自動填入模組，具備精確控制 XML 格式、嵌入 516*292 圖片與排版能力。
  * 成功自動填入並產出本日兩篇 Word 上稿單成果。
